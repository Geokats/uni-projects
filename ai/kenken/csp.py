...
def first_unassigned_variable(assignment, csp, forward_checking):
    "The default variable order."
    return first([var for var in csp.variables if var not in assignment])


def mrv(assignment, csp, forward_checking):
    "Minimum-remaining-values heuristic."
    return argmin_random_tie(
        [v for v in csp.variables if v not in assignment],
        key=lambda var: num_legal_values(csp, var, assignment,forward_checking))
...
def backtracking_search(csp,
    ...
    def backtrack(assignment):
        if len(assignment) == len(csp.variables):
            return assignment
        if select_unassigned_variable == backtracking_search:
            var = select_unassigned_variable(assignment, csp, True)
        else:
            var = select_unassigned_variable(assignment, csp, False)
